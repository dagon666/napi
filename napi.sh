...
verify_extention() {
    ...
    for ext in 'avi' 'rmvb' 'mov' 'mp4' 'mpg' 'mkv' 'mpeg' 'wmv'; do
        [[ "$ext" = "$extention" ]] && is_video=1 && break
    done
...
usage() {
	...
	local subotage_presence=${1:-0}
	local iconv_presence=${2:-0}
	local fpstool_presence=${3:-0}

	local identification=${4:-"unknown"}
	local orig_prefix=${5:-"ORIG_"}

	echo "=============================================================="
	echo "napi.sh version $g_Revison (identifies as $identification)"
	echo "napi.sh [OPCJE] <plik|katalog|*>"
	echo "   -c | --cover - pobierz okladke"

	[[ $iconv_presence -eq 1 ]] && echo "   -C | --charset - konwertuj kodowanie plikow (iconv -l - lista dostepnych kodowan)"
	...
	if [[ $subotage_presence -eq 1 ]]; then    
		...
		echo "   -o | --orig-prefix - prefix dla oryginalnego pliku przed konwersja (domyslnie: $orig_prefix)"   
